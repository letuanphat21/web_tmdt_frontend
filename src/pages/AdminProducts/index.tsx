...
// Dữ liệu giả lập (Mock data) có chứa link hình ảnh chiếc quần Jeans
const mockProducts = Array(5).fill({
    image: "https://images.unsplash.com/photo-1542272604-787c3835535d?q=80&w=300&auto=format&fit=crop", // Ảnh placeholder quần jeans
...
const AdminProducts = () => {
    ...
    return (
        <div className="flex flex-col gap-8 w-full max-w-6xl mx-auto pt-4 pb-12">
            ...
            {/* 1. Breadcrumb (Điều hướng 3 cấp giống Figma) */}
            <div className="flex items-center gap-3 text-sm">
                <span className="text-gray-500">Kênh ADMIN</span>
                <span className="text-gray-400 font-bold">›</span>
                <span className="text-gray-500">Quản lý sản phẩm</span>
                <span className="text-gray-400 font-bold">›</span>
                <span className="font-bold text-[#1A1C19]">Danh sách sản phẩm</span>
            </div>
            ...
            <div className="flex justify-between items-center px-2">
                ...
                {/* Khối bên phải: Sắp xếp + Nút Create */}
                <div className="flex items-center gap-6">
                    ...
                    <button className="px-8 py-2.5 bg-[#1A1C19] text-white rounded-lg font-bold hover:bg-black transition-colors shadow-sm">
                        Create
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] overflow-hidden border border-gray-100">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-white">
                    <tr className="border-b border-gray-100">
                        <th className="py-5 px-6 text-sm font-medium text-gray-500 whitespace-nowrap">Sản Phẩm</th>
                        <th className="py-5 px-6 text-sm font-medium text-gray-500 whitespace-nowrap text-center">Tên Sản Phẩm</th>
                    ...
                    {mockProducts.map((product, index) => (
                        <tr key={index} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                            ...
                            <td className="py-3 px-6">
                                <div className="w-[120px] h-[80px] overflow-hidden rounded-md shadow-sm border border-gray-100">
                                    <img
                                        src={product.image}
                                        alt={product.name}
                            ...
                            <td className="py-6 px-6 text-sm font-bold text-[#1A1C19] text-center">{product.name}</td>
                            <td className="py-6 px-6 text-sm text-gray-500 text-center">{product.id}</td>
                            <td className="py-6 px-6 text-sm text-gray-500 text-center">{product.owner}</td>
                            <td className="py-6 px-6 text-sm text-gray-500 text-center">{product.date}</td>
                            <td className="py-6 px-6 text-sm text-gray-500 text-center">{product.status}</td>
                            <td className="py-6 px-6 text-sm text-center">
                                {/* Chỗ này tương lai có thể gắn icon Edit/Delete */}
                            </td>
                        </tr>
                    ))}
                    </tbody>
                    ...
                    {[1, 2, 3, 4, 5].map((page) => (
                        <button
                            ...
                            className={`w-10 h-10 flex items-center justify-center rounded-full border text-sm font-medium transition-colors
                                ${page === 1
                                ? "border-[#49613E] text-[#49613E] font-bold"
                                : "border-[#1A1C19] text-[#1A1C19] hover:bg-gray-100"
                            }
                            `}
                        >
                            {page}
