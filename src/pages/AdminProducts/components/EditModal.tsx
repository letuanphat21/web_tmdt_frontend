...
function EditModal({ productId, setEditModal, setSuccess, setData }: any) {
  ...
  const handleChange = <K extends keyof any>(field: K, value: any[K]) => {
    ...
    try {
      const uploadedImages = await Promise.all(
        images.map(async (img: any) => {
          ...
          return {
            maHinhAnh: img.maHinhAnh,
            tenAnh: img.tenHinhAnh,
            duongDan: data.publicUrl,
          };
        }),
      );
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
          ...
          <form onSubmit={(e) => submit(e)} className="space-y-5 px-6 py-5">
            {/* {error && (
