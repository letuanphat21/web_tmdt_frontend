...
function UserProductSell() {
  ...
  const [editingProduct, setEditingProduct] = useState<SellProduct | null>(
    null,
  );
  ...
  const updateProductInList = (
    id: number,
    updater: (p: SellProduct) => SellProduct,
  ) => {
  ...
  const handleToggleActive = async (product: SellProduct) => {
    ...
    updateProductInList(product.id, (p) =>
      patchSellProduct(p, { active: nextActive }),
    );
    ...
    try {
      ...
      updateProductInList(product.id, () => previous);
      setError("Không cập nhật được trạng thái đăng bán. Vui lòng thử lại.");
    } finally {
      setTogglingId(null);
    }
  };
  ...
  const handleRestock = async (product: SellProduct, addQuantity: number) => {
    const newQty = product.inStock + addQuantity;
    const previous = product;

    updateProductInList(product.id, (p) =>
      patchSellProduct(p, { inStock: newQty }),
    );

    setRestockingId(product.id);
    try {
      await updateSellerProduct(product.id, { soLuong: newQty });
      if (filter === "SOLD OUT") {
        await loadProducts(true);
      }
    } catch {
      updateProductInList(product.id, () => previous);
      setError("Không nhập kho được. Vui lòng thử lại.");
    } finally {
  ...
  const handleEditSaved = (updated: SellProduct, imagesChanged: boolean) => {
    ...
    if (
      imagesChanged ||
      (filter === "SOLD OUT" && !updated.isSoldOut) ||
      (filter === "ACTIVE" && updated.isSoldOut)
    ) {
      loadProducts(true);
    }
  };
      ...
      <div className="mx-auto max-w-6xl px-6">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Quản lý và lọc sản phẩm bạn đã đăng
          </p>
        </div>
            ...
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                filter === key
                  ...
                  : "bg-green-100 text-brand-primary hover:bg-green-200/80"
              }`}
            >
              {label}
            </button>
          ...
          {initialLoading ? (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: PAGE_SIZE }).map((_, i) => (
                <div
                  key={i}
                  className="h-105 animate-pulse rounded-xl bg-white/80 shadow-sm"
                />
              ))}
            </div>
          ) : (
