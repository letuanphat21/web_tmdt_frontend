...
import {
  patchSellProduct,
  updateSellerProduct,
  type ProductImageSeller,
  type ProductImageUpdatePayload,
  type SellProduct,
} from "@/services/productSellerService";
import UserProductSellImageEditor, {
  countVisibleImages,
  type ExistingImageEdit,
  type NewImageEdit,
} from "./UserProductSellImageEditor";

type Props = {
  product: SellProduct;
  onClose: () => void;
  onSaved: (updated: SellProduct, imagesChanged: boolean) => void;
};
...
async function buildImagePayload(state: ImageEditorState) {
  const deleteImageIds = state.existing
  ...
  for (const img of state.existing) {
    ...
    if (img.replaceFile) {
      ...
      images.push({
        ...
        tenAnh: img.replaceFile.name,
        duongDan,
      });
      finalImages.push({
        maHinhAnh: img.maHinhAnh,
        tenHinhAnh: img.replaceFile.name,
        duongDan,
      });
    } else {
      finalImages.push({
  ...
  for (const neu of state.added) {
    ...
    images.push({
      maHinhAnh: 0,
      tenAnh: neu.file.name,
      duongDan,
    });
    finalImages.push({
      maHinhAnh: 0,
      tenHinhAnh: neu.file.name,
      duongDan,
    });
  }
  ...
  return {
    ...
    finalImages,
    changed:
      deleteImageIds.length > 0 || images.length > 0 || state.added.length > 0,
  };
}

function UserProductSellEditModal({ product, onClose, onSaved }: Props) {
  const [form, setForm] = useState<UploadProduct>({
    tenSanPham: product.name,
    soLuong: product.inStock,
    giaBan: product.priceValue,
    moTa: product.moTa,
    mauSac: product.mauSac,
    kichThuoc: product.kichCo,
    thuongHieu: product.thuongHieu,
    categoryId: product.maTheLoai,
    tinhTrangId: product.maTinhTrang,
    images: [],
  });
  ...
  const handleImageChange = useCallback((state: ImageEditorState) => {
    setImageState(state);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.soLuong < product.soLuongDaBan) {
      setError(
        `Số lượng không được nhỏ hơn số đã bán (${product.soLuongDaBan}).`,
      );
      return;
    }

    if (!imageState) {
      setError("Ảnh chưa sẵn sàng. Vui lòng thử lại.");
      return;
    }

    const visibleCount = countVisibleImages(
      imageState.existing,
      imageState.added,
    );
    if (visibleCount < 1) {
    ...
    if (visibleCount > 3) {
      setError("Tối đa 3 ảnh cho mỗi sản phẩm.");
      return;
    }
    ...
    try {
      ...
      await updateSellerProduct(product.id, {
        ...
        tinhTrangId: form.tinhTrangId,
        ...(imagePayload.deleteImageIds.length > 0 && {
          deleteImageIds: imagePayload.deleteImageIds,
        }),
        ...(imagePayload.images.length > 0 && {
          images: imagePayload.images,
        }),
      });

      onSaved(
        patchSellProduct(product, {
          name: form.tenSanPham.trim(),
          inStock: form.soLuong,
          priceValue: form.giaBan,
          moTa: form.moTa.trim(),
          mauSac: form.mauSac.trim(),
          kichCo: form.kichThuoc.trim(),
          thuongHieu: form.thuongHieu.trim(),
          maTheLoai: form.categoryId,
          maTinhTrang: form.tinhTrangId,
          images: imagePayload.finalImages,
          image: imagePayload.finalImages[0]?.duongDan ?? product.image,
        }),
        imagePayload.changed,
      );
      onClose();
    } catch (err: unknown) {
      const msg =
        err instanceof Error ? err.message : "Không lưu được thay đổi.";
      setError(
        msg.includes("Upload") || msg.includes("ảnh")
          ? msg
          ...
          <form onSubmit={handleSubmit} className="space-y-5 px-6 py-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  type="number"
                  min={product.soLuongDaBan}
                  value={form.soLuong}
                  onChange={(e) =>
                    handleChange("soLuong", Math.max(0, Number(e.target.value)))
                  }
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-primary focus:outline-none"
                />
                {product.soLuongDaBan > 0 && (
                  <p className="text-xs text-gray-400">
                    Tối thiểu: {product.soLuongDaBan} (đã bán)
                  </p>
                )}
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-primary focus:outline-none"
              />
            </div>
            ...
            <div className="flex justify-end gap-3 border-t border-gray-100 pt-4">
              <button
                type="button"
                onClick={onClose}
                className="rounded-full border border-gray-200 px-5 py-2 text-sm text-gray-700 hover:bg-gray-50"
              >
                Hủy
              </button>
              <button
                type="submit"
                disabled={saving}
                className="rounded-full bg-brand-primary px-5 py-2 text-sm font-medium text-white hover:bg-[#3d4938] disabled:opacity-60"
              >
